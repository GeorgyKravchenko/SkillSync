...
export default function TopicPage() {
  ...
  const title = topicTitles[slug];
  const { data } = usePosts(slug);
  useEffect(() => {
    console.log(data);
  }, [data]);

  if (!slug || !title) return notFound();
  ...
  return (
    <div className="pt-24 min-h-screen bg-gradient-to-br from-cyan-50 to-blue-100 dark:from-gray-900 dark:to-gray-950 text-gray-900 dark:text-white p-10 font-sans">
      <header className="mb-12 text-center">
        <h1 className="text-4xl font-extrabold mb-2 drop-shadow-[0_0_8px_cyan]">{title}</h1>
        <p className="text-cyan-800 dark:text-cyan-300 max-w-2xl mx-auto text-lg">
          {`Огляд теми "${title}" — статті, обговорення, ресурси та новини.`}
        </p>
        <div className="mt-6">
          <Link
            href={`/topic/${slug}/new`}
